...
import { liveChatroomsState, LiveChatroomsStateProps } from "../../state/liveChatRooms"
...
import { fetchLiveChatRooms } from "../../network/apis"
...
const LiveChatRoomListScreen = () => {
    ...
    useEffect(() => {
        let temp = data?.data
        data?.data.rooms.sort((a, b) => new Date(b?._updatedAt)?.getTime?.() - new Date(a?._updatedAt)?.getTime?.())
    ...
    const roomChangeEventHandler = (value: RoomChangeProps) => {
        ...
        if (roomIdx != undefined && roomIdx != -1) {
            ...
            if (temp) {
                console.log(temp.rooms[roomIdx], value.fields.args[1])
    ...
    console.log(liveChatRoomsData?.rooms)
    return (
