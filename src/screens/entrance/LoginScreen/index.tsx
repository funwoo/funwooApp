...
const LoginScreen = () => {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      const {data: result} = await backyardAPIHttpClient.post(
        '/auth/google/auth/pwd',
        {
          pwd: data.password,
          usr: data.user,
        },
  ...
  const onGoogleSignInPress = async () => {
    try {
      const deepLinkUrl =
        'com.googleusercontent.apps.120470042915-bjcbrvkvn1818rep0j0sg536fcvu3tto://';
      const {
        data: {authURL},
      } = await backyardAPIHttpClient.get('/auth/google/issue', {
        params: {
          state: deepLinkUrl,
        },
        headers: {
      ...
      if (result.type === 'success') {
        ...
        const {data} = await backyardAPIHttpClient.post(
          '/auth/google/auth/mobile',
          {
            code: query.code,
            scope: query.scope,
      ...
      <View style={{flex: 3}}>
        <TouchableOpacity
          ...
          style={{
            ...
            alignItems: 'center',
            backgroundColor: isValid ? 'black' : '#9e9e9e',
            marginTop: 40,
          }}>
          {isSubmitting ? (
            <ActivityIndicator />
        ...
        <View>
          <Pressable
            ...
            <FastImage
              ...
              source={{
                uri: 'https://cdn.funwoo.com.tw/inventory/funwoo_assets/2e0e4a88f775c0eb34b344400b314aa2593eca4b.jpeg',
              }}
            />
