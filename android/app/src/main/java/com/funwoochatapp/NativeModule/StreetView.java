...
public class StreetView  extends StreetViewPanoramaView implements OnStreetViewPanoramaReadyCallback {
            ...
            public void onStreetViewPanoramaCameraChange(StreetViewPanoramaCamera streetViewPanoramaCamera) {
                if (!(streetViewPanoramaCamera.bearing >= 0 ) && coordinate != null) {
                    eventDispatcher.dispatchEvent(
                            new StreetViewEvent(getId(), StreetViewEvent.ON_ERROR)
                    );
                }
            }
        });
        ...
        panorama.setOnStreetViewPanoramaChangeListener(new StreetViewPanorama.OnStreetViewPanoramaChangeListener() {
            ...
            public void onStreetViewPanoramaChange(StreetViewPanoramaLocation streetViewPanoramaLocation) {
                if (streetViewPanoramaLocation != null) {
                    ...
                    eventDispatcher.dispatchEvent(
                            new StreetViewEvent(getId(), StreetViewEvent.ON_SUCCESS, map)
                    );
                } else {
                    eventDispatcher.dispatchEvent(
                            new StreetViewEvent(getId(), StreetViewEvent.ON_ERROR)
    ...
    public void setMarker(ReadableMap coordinate){
        if (coordinate == null ) return;
        Double lng = coordinate.getDouble("longitude");
        Double lat = coordinate.getDouble("latitude");
    }
